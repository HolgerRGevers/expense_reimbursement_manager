...
## DL-007: Custom API Builder — initial discovery and research

**Date**: 2026-04-09
**Status**: OPEN (needs Creator verification)
**Source**: Creator UI — Developer Guide > Microservices > Custom API

**What we found**: Zoho Creator has a **Custom API Builder** feature that allows defining custom REST API endpoints backed by Deluge scripts. This is different from the REST API v2.1 (which does fixed CRUD on forms/reports). Custom APIs execute arbitrary Deluge logic and return user-defined JSON responses.

**The 5-step wizard**:
1. Basic Details (Display Name, Link Name, Description)
2. Request (input parameters with data types)
3. Response (output structure with data types)
4. Actions (Deluge script — the business logic)
5. Summary (review and publish)

**What's UNCERTAIN** (needs verification in Creator UI):
- Exact endpoint URL format generated by the builder
- OAuth scope required for Custom API execution
- How request parameters are accessed in the Actions Deluge script
- How response values are populated in the Actions Deluge script
- Whether Custom APIs appear in .ds exports
- Whether Free Trial has Custom API Builder access
- Whether Custom API calls share the standard API daily quota

**Impact**: New script context type (`custom-api`) added to manifest, scaffold, and linter. Research document created at `docs/zoho-custom-api-builder-research.md`.

**Actions taken**:
- Created comprehensive research document with use case mapping
- Added `custom-api` context to `config/deluge-manifest.yaml`
- Added scaffold template to `tools/scaffold_deluge.py`
- Added linter rules DG020/DG021 for Custom API context
- Added Custom API section to `config/deluge-reference.md`
- Updated `enhancements/future-roadmap.md` with Custom API section

**Next steps**:
1. Create a test Custom API in Creator UI (`Test_Ping`) to verify wizard and endpoint format
2. Document the exact endpoint URL, parameter access syntax, and response construction
3. Test OAuth scope requirements
4. Check .ds export behavior with Custom APIs
5. Build first real Custom API (`Get_Dashboard_Summary`) once patterns are verified
